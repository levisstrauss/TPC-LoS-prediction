...
from models.focal_msle import FocalMSLE
...
class TempPointConv(nn.Module):
    def __init__(self, config, F=None, D=None, no_flat_features=None):
        ...
        self.focal_msle = FocalMSLE(gamma=config.gamma)
    ...
    def loss(self, y_hat_los, y_hat_mort, y_los, y_mort, mask, seq_lengths, device, sum_losses, loss_type):
        ...
        # los loss
            ...
            elif loss_type == 'focal_msle':
                los_loss = self.focal_msle(y_hat_los, y_los, mask.type(bool_type), seq_lengths, sum_losses)
            if self.task == 'LoS':
                loss = los_loss
            # multitask loss
